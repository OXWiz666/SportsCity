...
export default function FeaturesSection() {
    ...
    return (
        <section id="features" className="py-24 md:py-32 bg-zinc-950 relative overflow-hidden text-left border-y border-zinc-900">
            {/* Background design */}
            <div className="absolute top-0 right-0 w-1/3 h-full bg-zinc-900 transform skew-x-[-20deg] origin-top opacity-50 z-0 pointer-events-none border-l border-zinc-800/50"></div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {features.map((feature, index) => (
                        <article 
                            key={index}
                            className="bg-black border-2 border-zinc-800 flex flex-col group overflow-hidden hover:border-lime-400 transition-colors duration-300 transform md:-skew-x-[2deg] hover:-translate-y-1 shadow-2xl shadow-black/50"
                        >
                            {/* Feature Thumbnail Coverage */}
                            <figure className="w-full aspect-[16/10] overflow-hidden relative m-0 border-b-2 border-zinc-800 group-hover:border-lime-400 transition-colors duration-300 md:skew-x-[2deg] scale-105">
                                <div className="absolute inset-0 bg-black/60 z-10 group-hover:bg-lime-400/10 transition-colors duration-300 mix-blend-overlay"></div>
                                <img 
                                    src={feature.image} 
                                    alt={feature.title} 
                                    loading="lazy"
                                ...
                                />
                                <div className={`absolute top-4 left-4 p-3 bg-black border border-zinc-700/50 z-20 group-hover:bg-lime-400 group-hover:border-lime-400 transition-colors duration-300 skew-x-[-10deg]`}>
                                    <feature.icon className={`w-6 h-6 text-lime-400 group-hover:text-black transition-colors duration-300 skew-x-[10deg]`} />
                                </div>
                            </figure>
                            
                            {/* Feature Body */}
                            <div className="p-8 pb-10 flex-grow flex flex-col md:skew-x-[2deg]">
