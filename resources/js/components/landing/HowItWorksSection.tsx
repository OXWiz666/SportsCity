...
export default function HowItWorksSection() {
    ...
    return (
        <section id="how-it-works" className="py-24 md:py-32 bg-black relative border-y-4 border-lime-400 overflow-hidden">
            {/* Caution tape background elements */}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-8 relative z-10">
                    {steps.map((step, index) => (
                        <div key={index} className="relative flex flex-col text-left group border-2 border-zinc-900 bg-zinc-950 p-8 hover:bg-zinc-900 transition-colors duration-300 hover:border-lime-400">
                            
                            <div className="absolute -top-10 right-6 text-8xl font-black opacity-10 text-white group-hover:opacity-100 group-hover:text-lime-400 transition-colors duration-300 select-none">
                                {step.step}
                            </div>
                            
                            <div className="w-16 h-16 flex items-center justify-center bg-black border-[3px] border-lime-400 mb-8 z-10 transform skew-x-[-15deg] group-hover:bg-lime-400 transition-colors duration-300">
                                <step.icon className="w-8 h-8 text-lime-400 transform skew-x-[15deg] group-hover:text-black transition-colors duration-300" />
                            </div>
                            
                            <h4 className="text-3xl font-black text-white mb-4 tracking-tighter uppercase">{step.title}</h4>
            ...
            <div className="absolute top-0 left-0 w-16 h-full bg-[repeating-linear-gradient(45deg,#000,#000_15px,#27272a_15px,#27272a_30px)] opacity-80 z-0 pointer-events-none"></div>
            <div className="absolute top-0 right-0 w-16 h-full bg-[repeating-linear-gradient(-45deg,#000,#000_15px,#27272a_15px,#27272a_30px)] opacity-80 z-0 pointer-events-none"></div>
        </section>
    );
}
